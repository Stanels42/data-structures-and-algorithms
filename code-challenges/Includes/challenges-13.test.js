...
const standardizePhoneNumbers = (arr) => {
  return arr.map(str => {
    ...
    return charArr.filter(char => /\d/.test(char)).join('');
  });
};
...
const sortByDay = (arr) => {
  ...
  arr.forEach(day => {
    ...
    if((/monday/gi).test(day)) {result[0].push(day);}
    if((/tuesday/gi).test(day)) {result[1].push(day);}
    if((/wednesday/gi).test(day)) {result[2].push(day);}
    if((/thursday/gi).test(day)) {result[3].push(day);}
    if((/friday/gi).test(day)) {result[4].push(day);}
    if((/saturday/gi).test(day)) {result[5].push(day);}
    if((/sunday/gi).test(day)) {result[6].push(day);}
